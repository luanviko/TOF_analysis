...
def read_config_file(config_file):
    ## Loads a config file containing the information 
    ## that otherwhise would be hard-coded here.
    ## INPUT:  _config_file_
    ## OUTPUT: _config_dictionary
    config_parameters = {}
    with open(config_file, "r") as config_input:
        config_lines = config_input.readlines()
            ...
            if len(line) > 1:
                if line[0] != '#':
                    ...
                    if '#' not in line_split[1]:
                        config_parameters.update({line_split[0]:line_split[1].replace(" ","")})
                    else:
                        line_split_again = line_split[1].split('#')
                        config_parameters.update({line_split[0]:line_split_again[0].replace(" ","")})
    return config_parameters

def simple_waveform_debug(event, label, waveform, nSamples):
    x = np.arange(0,nSamples)
    fig, axi = plt.subplots(nrows=1, ncols=1, figsize=(6,6))
    axi.set_xlim([0,nSamples])
    axi.set_xlabel("Time (ns)")
    axi.set_ylabel("Amplitude (mV)")
    axi.plot(x,waveform[0:nSamples])
    plt.tight_layout()
    plt.savefig(f"./linear_test/waveform-{event}_{label}.png")

def plot_waveform(event, label, waveform, start_rise_time, end_rise_time, a, b, timing, nSamples):
...
def find_pulse_information(run_number, key, event, waveform, start_rise, end_rise, percentage, nSamples, noise_threshold, horizontalScaleFactor, plot_waveforms):
    ...
    if plot_waveforms == "True":
        plot_waveform(event, "interpolation", waveform, i_start, i_end, a, b, imax00_CFD, nSamples)
...
def save_txt(run_number, percentage, key, CFD_times, global_times, amplitudes, charges, output_path):
    ...
    with open(f"{output_path}/pulse_information-{run_number}-{percentage}rise_time-{key}.txt","w") as file_output:
...
# Find parameters
if len(sys.argv) != 4:
    print("How to use: python TOF_analysis_8channels.py _output.root_ _run_number_" "_config_file")
    sys.exit(1)

# Fetch config parameters
config_parameters = read_config_file(sys.argv[3])
# print(config_parameters)
# sys.exit(0)

# Digitizer specs
mppc_DIGITIZER_FULL_SCALE_RANGE = float(config_parameters["mppc_DIGITIZER_FULL_SCALE_RANGE"])
mppc_DIGITIZER_RESOLUTION       = int(config_parameters["mppc_DIGITIZER_RESOLUTION"])
mppc_DIGITIZER_SAMPLE_RATE      = int(config_parameters["mppc_DIGITIZER_SAMPLE_RATE"])
nSamples = int(config_parameters["nSamples"])
...
percentage = float(config_parameters["percentage"])
start_rise = float(config_parameters["start_rise"])
end_rise = float(config_parameters["end_rise"])
noise_threshold = float(config_parameters["noise_threshold"])
plot_waveforms = config_parameters["plot_waveforms"]
output_path = config_parameters["output_path"]
...
for key in waveforms.keys():
    ...
    for i in range(0, len(waveforms[key])):
        ...
        timing_CFD, timing_global, amplitude, charge = find_pulse_information(run_number, key, i, channel_waveforms[i], start_rise, end_rise, percentage, nSamples, noise_threshold, horizontalScaleFactor, plot_waveforms)
    ...
    save_txt(run_number, percentage, key, CFD_times, global_times, amplitudes, charges, output_path)
